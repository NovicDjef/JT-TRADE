      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {destinations.map((dest, index) => (
            <motion.div key={dest.country} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.5, delay: index * 0.1 }}>
              <Link href="/contact" className="group block overflow-hidden rounded-2xl bg-white shadow-md hover:shadow-xl transition-all duration-300">
                ...
                <div className="p-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {dest.services.map((s) => (
                      <span key={s} className="px-2 py-1 bg-red-50 text-red-600 text-xs rounded-md font-medium">{s}</span>
                    ))}
                  </div>
                </div>
              </Link>
